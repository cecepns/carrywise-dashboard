...
export const Sidenav: React.FC<SidenavProps> = memo(({ routes }) => {
  ...
  return (
    <aside
      className={classNames('bg-gradient-to-br from-gray-800 to-gray-900 fixed inset-0 z-50 my-4 ml-4 h-[calc(100vh-32px)] w-72 rounded-xl transition-transform duration-300 xl:translate-x-0', {
      ...
      <div className="m-4">
        {routes.map(({ layout, title, pages } : Layout, key:number) => layout === "dashboard" && (
          <ul key={key} className="mb-4 flex flex-col gap-1">
            ...
            {pages.map(({ icon, name, path }: PageRoutes) => (
              <li key={name}>
